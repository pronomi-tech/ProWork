    ...
    func fetchAll() throws -> [TodoListItem] {
        ...
        return try database.query(sql) { statement in
            TodoListItem(
                ...
                plannedDate: Self.parseDate(statement.text(at: 19)),
                dueDate: Self.parseDate(statement.text(at: 20)),
                estimatedMinutes: statement.optionalInt(at: 21),

                totalTrackedSeconds: statement.int(at: 22),
                activeSessionStartedAt: Self.parseDate(statement.text(at: 23)),

                isBillable: statement.int(at: 24) == 1,

                createdAt: Self.parseDate(statement.text(at: 25)) ?? Date(),
                updatedAt: Self.parseDate(statement.text(at: 26)) ?? Date(),
                completedAt: Self.parseDate(statement.text(at: 27))
            )
        }
    }
